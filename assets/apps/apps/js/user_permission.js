...
autoWidth: false,$(function () {
    ...
    $('#user_permission').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: base_url + 'user_permission/all',
            type: 'POST'
        },
        columns: [
    ...
    var table = $('#user_permission').DataTable();
    ...
    function createUpdate() {
        $.ajax({
            url: $('#form_user_permission').attr('action'),
            type: 'POST',
            dataType: 'json',
            data: new FormData($('#form_user_permission')[0]),
            processData: false,
            contentType: false,
            success: function(response) {
                if(response.status == '1'){
                    $('#bean_modal').modal('hide');
                    $('#user_permission').DataTable().ajax.reload();
                }
            }
        });
    }
    ...
    table.on('click', '.remove', function(e) {
        ...
        if (confirm("Are you sure?")) {
            $.ajax({
                ...
                dataType: 'json',
                success: function(response) {
                    (response.status == '1') ? $('#user_permission').DataTable().ajax.reload() : '';
                }
            });
        }
    });
